# Learning_How_To_Use_Github_Changing_The_Title
*While learning from the course 'Introduction to GitHub concepts by Datacamp' practicing*

**Writing a README**
* Need to be descriptive
* Anyone should understand our project
* List the contents of the repository
* Clearly explains the project to others

## Double hashes will includes a new sub heading

### Great it is working


[This way we can add hyperlinks to our Readme.md file](https://github.com/FidaHussain110/Renamed_Title_Its_an_Exercise/edit/main/README.md)


#### Pictures can also be added in README files
![Uploading image.png…](https://avatars.githubusercontent.com/u/215631737?v=4&size=64)
